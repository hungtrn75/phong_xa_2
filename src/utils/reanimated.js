...
export function runTiming(clock, value, dest, startStopClock = true) {
  ...
  const startStopClockAni = startStopClock ? new Value(1) : new Value(0);
  ...
  return [
    cond(clockRunning(clock), 0, [
      ...
      cond(startStopClockAni, startClock(clock)),
    ]),
    timing(clock, state, config),
    cond(and(state.finished, startStopClockAni), stopClock(clock)),
    state.position,
  ];
}
