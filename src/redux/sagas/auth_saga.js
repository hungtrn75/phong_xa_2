  ...
  try {
    ...
    if (isInternetReachable) {
      ...
      if (resLogin.access_token) {
        ...
        const [resMe, pendingFeatures, {layers, features}] = yield all([
          call(AuthService.me),
          call(LayerService.getPendingFeatures),
          call(LayerService.getDoneFeatures),
        ]);
        const [banDoNen, bookmarks] = yield all([
          call(LayerService.layBanDoNen),
        ...
        showMessage({
          ...
          description: 'Kiểm tra lại kết nối của bạn',
          type: 'danger',
...
function* getMeWorker() {
  try {
    ...
    let banDoNen = yield call(LayerService.layBanDoNen);
